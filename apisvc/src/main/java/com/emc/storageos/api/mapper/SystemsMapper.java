...
public class SystemsMapper {
	...
	public static SMISProviderRestRep mapStorageProviderToSMISRep(StorageProvider from) {
		...
		// Workaround to generate /vdc/smis-providers uri for self link instead
		// of type base URI generation.
		try {
			to.setLink(new RestLinkRep("self",
					RestLinkFactory.simpleServiceLink(ResourceTypeEnum.SMIS_PROVIDER, from.getId())));
	...
	public static VirtualNASRestRep map(VirtualNAS from, DbClient dbClient) {
		...
		Double usedStorageCapacity = MetricsKeys.getDouble(MetricsKeys.usedStorageCapacity, from.getMetrics())
				/ GBsINKB;
	...
	public static StoragePoolRestRep map(StoragePool from, Map<String, Long> capacityMetrics,
		...
		if ((null != capacityMetrics.get(CapacityUtils.StorageMetrics.SUBSCRIBED.toString()) && !(capacityMetrics
				.get(CapacityUtils.StorageMetrics.SUBSCRIBED.toString()).toString().equals(MINUS_ONE_LONG)))) {
		...
		if (null != from.getSupportedResourceTypes()
			...
			to.setMaxThinPoolSubscriptionPercentage(
					(from.getMaxThinPoolSubscriptionPercentage() != null) ? from.getMaxThinPoolSubscriptionPercentage()
							: Integer.valueOf(ControllerUtils.getPropertyValueFromCoordinator(coordinatorClient,
